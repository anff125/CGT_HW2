...
void initialize_device_constants() {
    cudaMemcpyToSymbol(d_movable_piece_table, movable_piece_table, sizeof(movable_piece_table));
    cudaMemcpyToSymbol(d_step_table, step_table, sizeof(step_table));
}
...
{
    int move_id = id_with_dice / PIECE_NUM; // index of moves[][]
    dice = id_with_dice % PIECE_NUM;        // opponent's dice
...
__host__ __device__ char Board::check_winner()
{
